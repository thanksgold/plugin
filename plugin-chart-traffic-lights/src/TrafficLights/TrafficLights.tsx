...
class TrafficLightsVis extends React.PureComponent<TrafficLightsVisProps, {}> {
  ...
  getClassName() {
    const { className, showTrendLine, trafficLightsFallback } = this.props;
    const names = `superset-plugin-chart-traffic-lights ${className} ${
      trafficLightsFallback ? 'is-fallback-value' : ''
    }`;
  ...
  renderFallbackWarning() {
    const { trafficLightsFallback, formatTime } = this.props;
    if (!trafficLightsFallback) return null;
    return (
      <span
        className="alert alert-warning"
        role="alert"
        title={t(`Last available value seen on %s`, formatTime(trafficLightsFallback.x))}
      >
        {t('Not up to date')}
      </span>
    );
  ...
  renderHeader(maxHeight: number) {
    const { trafficLights, formatNumber, width } = this.props;
    const text = trafficLights === null ? t('No data') : formatNumber(trafficLights);
  ...
  renderSubheader(maxHeight: number) {
    const { trafficLights, subheader, width, trafficLightsFallback } = this.props;
    let fontSize = 0;
...
export default styled(TrafficLightsVis)`
