...
// we trust both the x (time) and y (traffic lights) to be numeric
export interface TrafficLightsDatum {
  [key: string]: number | null;
}

export type TrafficLightsFormData = QueryFormData & {
  colorPicker?: {
    r: number;
    g: number;
    b: number;
...
export type TrafficLightsChartProps = ChartProps & {
  formData: TrafficLightsFormData;
  queriesData: (LegacyQueryData & {
    data?: TrafficLightsDatum[];
  })[];
};
...
export default function transformProps(chartProps: TrafficLightsChartProps) {
  ...
  const supportTrendLine = vizType === 'traffic_lights';
  ...
  let trendLineData;
  let percentChange = 0;
  let trafficLights = data.length === 0 ? null : data[0][metricName];
  let trafficLightsFallback;

  if (data.length > 0) {
    const sortedData = (data as TrafficLightsDatum[])
      .map(d => ({ x: d[TIME_COLUMN], y: d[metricName] }))
      // sort in time descending order
      .sort((a, b) => (a.x !== null && b.x !== null ? b.x - a.x : 0));

    trafficLights = sortedData[0].y;
    if (trafficLights === null) {
      trafficLightsFallback = sortedData.find(d => d.y !== null);
      trafficLights = trafficLightsFallback ? trafficLightsFallback.y : null;
    }
    ...
    if (compareLag > 0) {
      ...
      if (compareIndex < sortedData.length) {
        ...
        if (trafficLights !== null && compareValue !== null && compareValue !== 0) {
          percentChange = (trafficLights - compareValue) / Math.abs(compareValue);
